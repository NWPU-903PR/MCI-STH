...
def train(model, train_loader, device, optimizer, classification_loss,  weight_ELBO = 1, weight_MSE = 1, weight_KL = 1,\
          weight_CMG = 1, weight_MMC = 1, weight_AC_PET = 1, weight_AC_SNP = 1, use_mask_rate=0.2):
    ...
    for it, train_data in enumerate(train_loader):
        for data_dic in train_data:
            ...
            SNP_mu_rg, SNP_logvar_rg, SNP_z_rg, PET_y, SNP_y, muti_y, muti_z_f, mask_PET, mask_SNP = out


            has_pet = label_PET.sum() > 0
            has_snp = label_SNP.sum() > 0
            ...
            if has_pet:
                ...
                loss_rg_PET_mu = loss_rg_PET_logvar = torch.tensor(0.0, device=device)
            ...
            if has_snp:
                ...
                loss_rg_SNP_mu = loss_rg_SNP_logvar = torch.tensor(0.0, device=device)
            ...
            if has_pet:
                ...
                loss_AC_PET = torch.tensor(0.0, device=device)
            train_loss_AC_PET += loss_AC_PET

            if has_snp:
                loss_AC_SNP = classification_loss(SNP_y, true_labels)
            else:
                loss_AC_SNP = torch.tensor(0.0, device=device)
