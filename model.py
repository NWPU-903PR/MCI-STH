...
class VAE_all(nn.Module):
    ...
    def forward(self, x1, x2, x2_label, x3, x3_label, device, training=False, use_mask_rate=0):
        batch_size = x1.size(0)

        mask_PET = torch.zeros(batch_size).to(device)
        mask_SNP = torch.zeros(batch_size).to(device)

        if training and use_mask_rate > 0:
            valid_idx_PET = (x2_label == 1).nonzero(as_tuple=True)[0]
            num_mask_PET = int(len(valid_idx_PET) * use_mask_rate)

            if num_mask_PET > 0:
                perm_PET = torch.randperm(len(valid_idx_PET))
                selected_idx_PET = valid_idx_PET[perm_PET[:num_mask_PET]]
                mask_PET[selected_idx_PET] = 1

            valid_idx_SNP = (x3_label == 1).nonzero(as_tuple=True)[0]
            num_mask_SNP = int(len(valid_idx_SNP) * use_mask_rate)

            if num_mask_SNP > 0:
                perm_SNP = torch.randperm(len(valid_idx_SNP))
                selected_idx_SNP = valid_idx_SNP[perm_SNP[:num_mask_SNP]]
                mask_SNP[selected_idx_SNP] = 1

        PET_recon_x, PET_mu, PET_logvar, PET_z = [None] * batch_size, [None] * batch_size, [None] * batch_size, [
            None] * batch_size
        PET_mu_rg, PET_logvar_rg, PET_z_rg, PET_AM = [None] * batch_size, [None] * batch_size, [None] * batch_size, [
            None] * batch_size
        SNP_recon_x, SNP_mu, SNP_logvar, SNP_z = [None] * batch_size, [None] * batch_size, [None] * batch_size, [
            None] * batch_size
        SNP_mu_rg, SNP_logvar_rg, SNP_z_rg = [None] * batch_size, [None] * batch_size, [None] * batch_size
        ...
        for i in range(batch_size):
            if x2_label[i] == 1:
                real_recon, PET_mu[i], PET_logvar[i], PET_z[i], PET_AM[i] = self.VAE_all_PET(x2[i])

                PET_mu_rg[i], PET_logvar_rg[i], PET_z_rg[i] = self.RegressionModel_sMRI_PET(sMRI_mu[i], sMRI_logvar[i])

                if mask_PET[i] == 0:
                    PET_recon_x[i] = real_recon 
                else:
                    PET_recon_x[i] = self.VAE_all_PET.decoder_PET(PET_z_rg[i])
            else:
                ...
                PET_mu_rg[i], PET_logvar_rg[i], PET_z_rg[i] = [torch.zeros(16).to(device) for _ in range(3)]
                PET_AM[i] = torch.zeros((1, 91, 109, 91)).to(device)


        for i in range(batch_size):
            if x3_label[i] == 1:
                real_recon_snp, SNP_mu[i], SNP_logvar[i], SNP_z[i] = self.VAE_all_SNP(x3[i])
                SNP_mu_rg[i], SNP_logvar_rg[i], SNP_z_rg[i] = self.RegressionModel_sMRI_SNP(sMRI_mu[i], sMRI_logvar[i])

                if mask_SNP[i] == 0:
                    SNP_recon_x[i] = real_recon_snp
                else:
                    SNP_recon_x[i] = self.VAE_all_SNP.decoder_SNP(SNP_z_rg[i])
            else:
                ...
                SNP_mu_rg[i], SNP_logvar_rg[i], SNP_z_rg[i] = [torch.zeros(16).to(device) for _ in range(3)]
        ...
        return sMRI_recon_x, sMRI_mu, sMRI_logvar, sMRI_z, \
               ...
               SNP_mu_rg, SNP_logvar_rg, SNP_z_rg, PET_y, SNP_y, muti_y, muti_z_f, mask_PET, mask_SNP                #sMRI_AM, PET_AM
